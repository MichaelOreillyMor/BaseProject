GFFramework is a framework developed by Michael OReilly that tries to imitate the Unreal framework 
that works as a layer over the engine to speed up the games development.

The idea is to load in the GameInitializer only the managers needed in a similar way to Unreal modules.
...
GameStates system helps to create easily a game flow and know the UI that needs to be display in that moment.
